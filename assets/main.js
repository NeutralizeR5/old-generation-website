...
(() => {
  ...
  function setLang(l) {
    ...
    ['stats','profiles','terminology','news','shortfilm','contact'].forEach(id => {
      ...
      if(h) h.textContent = t[id+'_title'];
      const p = document.getElementById(id+'-text');
      if(p) p.textContent = t[id+'_text'];
    });

    // Haber alt başlıkları
  ...
  function fetchStats() {
    fetch('https://api.example.com/guild/stats')
      ...
      .then(data => {
        ...
        document.getElementById('stats-data').textContent = txt;
      })
      .catch(() => {
        document.getElementById('stats-data').textContent = translations[lang].error_stats;
      });
  }
